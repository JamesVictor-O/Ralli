...
import { fetchRalliById, removeMyRalli } from '../../lib/rallis.ts'
...
export function Profile({ onOpenRalli }: { onOpenRalli?: (ralli: Dare) => void }) {
  ...
  const [openingRalli, setOpeningRalli] = useState<string | null>(null)
  ...
  useDialogFocus(editDialogRef, editing, closeEditor)
  ...
  async function openRalli(id: string) {
    if (!onOpenRalli || openingRalli) return
    setOpeningRalli(id)
    setError('')
    try { onOpenRalli(await fetchRalliById(id)) }
    catch (failure) { setError(failure instanceof Error ? failure.message : 'This Ralli could not be opened.') }
    finally { setOpeningRalli(null) }
  }
  ...
  return <section className="page-view profile-page" aria-labelledby="profile-title">
    ...
    {error && !editing && <p className="payment-error" role="alert">{error}</p>}
    ...
    {tab === 'Rallis' && (data.rallis.length ? <div className="profile-list">{data.rallis.map((ralli) => <article key={ralli.id}><span className="rail-icon rail-icon--lime"><UsersRound aria-hidden="true" /></span><button className="profile-list__content" type="button" disabled={!onOpenRalli || openingRalli === ralli.id} aria-busy={openingRalli === ralli.id} onClick={() => void openRalli(ralli.id)}><strong>{ralli.prompt}</strong><small>{openingRalli === ralli.id ? 'Opening…' : `Ends ${new Date(ralli.ends_at).toLocaleDateString()}`}</small></button><button className="icon-button" type="button" aria-label={`Remove ${ralli.prompt}`} onClick={() => void removeRalli(ralli.id)}><Trash2 aria-hidden="true" /></button></article>)}</div> : <div className="empty-state"><Sparkles aria-hidden="true" /><h2>You haven’t started one yet</h2><p>Your Rallis will appear here after publishing.</p></div>)}
    ...
    {editing && <div className="wallet-backdrop wallet-backdrop--nested edit-profile-backdrop" role="dialog" aria-modal="true" aria-labelledby="edit-profile-title"><section className="payment-panel edit-profile-panel" ref={editDialogRef}><header className="wallet-panel__header edit-profile-panel__header"><div><p className="eyebrow">Your Ralli identity</p><h2 id="edit-profile-title">Edit profile</h2></div><button className="icon-button" type="button" aria-label="Close profile editor" disabled={saving} onClick={closeEditor}><X aria-hidden="true" /></button></header><form className="edit-profile-form" onSubmit={save}>
      <input className="sr-only" ref={avatarInputRef} type="file" accept="image/jpeg,image/png,image/webp" onChange={chooseAvatar} />
      <button className="avatar-picker edit-profile-avatar" type="button" onClick={() => avatarInputRef.current?.click()} aria-label={data.profile.avatar_path || avatarFile ? 'Change profile photo' : 'Add a profile photo'}>
        <span className="avatar-picker__media"><Avatar initials={initials} avatarUrl={avatarPreview || publicAvatarUrl(data.profile.avatar_path)} className="avatar-picker__preview" /><i><Image aria-hidden="true" /></i></span>
        <span className="avatar-picker__copy"><strong>{data.profile.avatar_path || avatarFile ? 'Change profile photo' : 'Add a profile photo'}</strong><small>A clear photo helps friends recognize you.</small></span>
        <span className="avatar-picker__action">{data.profile.avatar_path || avatarFile ? 'Change' : 'Add'}</span>
      </button>
      <div className="edit-profile-fields"><div className="field"><label htmlFor="display-name">Display name</label><input id="display-name" type="text" autoComplete="name" spellCheck={false} maxLength={60} value={displayName} onChange={(event) => setDisplayName(event.target.value)} /></div>
      <div className="field"><label htmlFor="profile-handle">Username <span>Optional</span></label><div className="input-with-icon"><strong>@</strong><input id="profile-handle" type="text" autoComplete="username" spellCheck={false} maxLength={24} placeholder="yourname" value={handle} onChange={(event) => setHandle(event.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))} /></div></div></div>
      <div className="field edit-profile-bio"><div className="field-meta"><label htmlFor="profile-bio">Bio</label><small>{bio.length}/240</small></div><textarea id="profile-bio" rows={3} maxLength={240} placeholder="What are you into?" value={bio} onChange={(event) => setBio(event.target.value)} /></div>
      {error && <p className="payment-error" role="alert">{error}</p>}
      <div className="edit-profile-form__actions"><button className="button button--soft" type="button" disabled={saving} onClick={closeEditor}>Cancel</button><button className="button button--ink" type="submit" disabled={saving} aria-busy={saving}>{saving && <LoaderCircle className="spin" aria-hidden="true" />}{saving ? 'Saving…' : 'Save changes'}</button></div>
    </form></section></div>}
  </section>
}
